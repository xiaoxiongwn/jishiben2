...
class ReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (reminderType != "none" && reminderType != "once") {
            scheduleNext(context, entryId, reminderType)
        }
    }
    ...
    private fun scheduleNext(context: Context, entryId: Long, reminderType: String) {
        CoroutineScope(Dispatchers.IO).launch {
            val db = DiaryDatabase.getDatabase(context)
            val entry = db.diaryDao().getById(entryId) ?: return@launch

            val nextTime = calculateNextReminder(entry.reminderTime, reminderType)
            if (nextTime > 0) {
    ...
    private fun calculateNextReminder(currentTime: Long, type: String): Long {
        ...
        return if (cal.timeInMillis > System.currentTimeMillis()) cal.timeInMillis else 0
    }
}
